...
import (
	"gitcode.deepway-inc.com/ed/ed-go/wms/service/gopdf/core"
	"math"
)
...
func NewTable(cols, rows int, width, lineHeight float64, pdf *core.Report, frameNone bool) *Table {
...
// 对当前的Page  跨页的内容 留到下一页了 所以超出部分不划线
func (table *Table) drawPageLines(sx, sy float64) {
	var (
		rows, cols              = table.rows, table.cols
		_, pageEndY             = table.pdf.GetPageEndXY()
		x, y, x1, y1, _, y2, y3 float64
	)
	...
	// 两条垂直线
	x, y, _, _ = table.getHLinePosition(sx, sy, 0, 0)
	table.pdf.LineV(x, y, y3)
	table.pdf.LineV(x+table.width, y, y3)
}
