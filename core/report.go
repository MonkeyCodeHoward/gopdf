...
import (
	"errors"
	"gitcode.deepway-inc.com/ed/ed-go/wms/internal/dlog"
	"gitcode.deepway-inc.com/ed/ed-go/wms/service/gopdf/util"
	"io/ioutil"
	"os"
	"regexp"
	"strconv"
...
func CreateReport(req interface{}) *Report {
	report := new(Report)
...
func (report *Report) Execute(filepath string) {
	...
	dlog.DaemonLogger().Info("pdf", "report", "4", 4, "report", report)
...
func (report *Report) executePageFooter() {
	...
	report.currY = report.pageEndY + 20
	report.currX = report.pageStartX
...
func (report *Report) SetPage(size string, orientation string) {
	...
	case "A4":
		...
		case "L":
			...
			report.pageStartX = config.startY
			report.pageStartY = config.startX

			report.pageWidth = config.height
			report.pageHeight = config.width

			report.pageEndX = config.endY
			report.pageEndY = config.endX

			report.contentWidth = config.contentHeight
			report.contentHeight = config.contentWidth

			//tem := config.startY
			//config.startY = config.startX
			//config.startX = tem
			//
			//tem = config.height
			...
			//
			//tem = config.endY
			//config.endY = config.endX
			//config.endX = tem
			//
			//tem = config.contentHeight
			//config.contentHeight = config.contentWidth
			//config.contentWidth = tem
